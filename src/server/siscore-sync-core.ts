...
const HMSA_ESTOQUES_CARRINHO_PARADA_CODE_SET = new Set<string>(HMSA_ESTOQUES_CARRINHO_PARADA_CODES);
...
function somarColunasEstoque(row: GenericRow, codes: readonly string[]) {
...
export function normalizarLinhasEstoque(rows: GenericRow[], categoriaMaterial: string) {
  const normalized = rows
    .map((row) => {
      ...
      const isHmsa = unidadeEhHmsa(codigoUnidade);
